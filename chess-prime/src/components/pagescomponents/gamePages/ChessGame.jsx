...
// Piece Image Component with fallback support
const PieceImage = ({ color, type, pieceSet }) => {
  const [imgError, setImgError] = useState(false);
  const [currentPathIndex, setCurrentPathIndex] = useState(0);
  
  // Map piece types to correct notation
  const pieceMap = {
    'k': 'k',
    'q': 'q',
    'r': 'r',
  ...
  const pieceType = pieceMap[type.toLowerCase()];
  const pieceColor = color === 'w' ? 'w' : 'b';
  const fileName = `${pieceColor}${pieceType.toUpperCase()}`;
  
  const getImagePaths = () => {
    return [
      `/assets/chesspieces/${pieceSet}/${fileName}.svg`,
      `/assets/chesspieces/${pieceSet}/${fileName}.png`,
      `/src/assets/chesspieces/${pieceSet}/${fileName}.svg`,
      `/src/assets/chesspieces/${pieceSet}/${fileName}.png`,
      `${API_BASE_URL}/assets/chesspieces/${pieceSet}/${fileName}.svg`,
      `${API_BASE_URL}/assets/chesspieces/${pieceSet}/${fileName}.png`,
      `${API_BASE_URL}/src/assets/chesspieces/${pieceSet}/${fileName}.svg`,
      `${API_BASE_URL}/src/assets/chesspieces/${pieceSet}/${fileName}.png`,
      // CDN fallback
      `https://cdn.jsdelivr.net/npm/chessboard-element@1.0.0/assets/pieces/${pieceSet}/${fileName}.svg`,
      // Ultimate fallback - null to trigger Unicode
      null
    ];
  };
  ...
  // Unicode piece characters as ultimate fallback
  const getUnicodePiece = () => {
    ...
    return pieces[key] || (color === 'w' ? '♙' : '♟');
  };
  
  if (imgError || !currentPath) {
    return (
      <div className="flex items-center justify-center w-full h-full text-3xl sm:text-4xl select-none">
        {getUnicodePiece()}
      </div>
    );
  }
...
export default function ChessGame({ gameState, onExit }) {
  ...
  // Load game moves for online games
  useEffect(() => {
    if (gameState?.gameMode === 'online' && gameId && !hasLoadedMovesRef.current) {
      loadGameMoves();
    }
  }, [loadGameMoves, gameState?.gameMode, gameId]);
                      ...
                      {availablePieceSets.slice(0, 6).map(set => (
                        ...
                        >
                          ...
                          <img
                            ...
                            onError={(e) => {
                              e.target.onerror = null;
                              e.target.src = `https://cdn.jsdelivr.net/npm/chessboard-element@1.0.0/assets/pieces/${set}/wN.svg`;
                            }}
                          />
                        </button>
                      ))}
