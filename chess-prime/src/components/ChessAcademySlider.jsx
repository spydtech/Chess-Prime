...
import slide2 from '../assets/landing-images/slider/hassan-pasha-WTZZomyqaoA-unsplash.jpg';
import slide3 from '../assets/landing-images/slider/piotr-makowski-27LH_0jXKYI-unsplash.jpg';
import BrandAmbassador from './BrandAmbassador'; // (now your updated neutral section)
import PricingSection from './PricingSection';
...
const ChessAcademySlider = () => {
  ...
  const slides = [
    {
      title: "Learn Chess the Smart Way",
      badge: "Structured Learning Program",
      stats: "Beginner → Advanced",
      description: "We help students improve focus, logic, and decision-making through chess.",
      image: slide1,
      overlay: "from-purple-900/90 to-indigo-900/90"
    },
    {
      ...
      stats: "1000+ Students",
      description: "Personalized coaching designed to improve gameplay and thinking ability.",
      image: slide2,
      overlay: "from-blue-900/90 to-purple-900/90"
    },
    {
      title: "Build Skills Beyond Chess",
      badge: "Holistic Development",
      stats: "All Age Groups",
      description: "Enhancing concentration, patience, and strategic thinking in every student.",
      image: slide3,
      overlay: "from-indigo-900/90 to-blue-900/90"
    }
  ];
  ...
  return (
    <div className="min-h-screen bg-[#150F0B] text-white">
      ...
      <div className="relative h-screen overflow-hidden">
          ...
          {slides.map((slide, index) => (
            <div key={index} className="w-full flex-shrink-0 relative h-full">
              ...
              >
                <div className={`absolute inset-0 bg-gradient-to-br ${slide.overlay}`}>
                  <div className="absolute inset-0 opacity-20">
                    <div className="absolute top-0 left-0 w-96 h-96 bg-white blur-3xl"></div>
                    <div className="absolute bottom-0 right-0 w-96 h-96 bg-yellow-500 blur-3xl"></div>
                  </div>
                </div>
              </div>
                ...
                <div className="container mx-auto px-4">
                  <div className="grid md:grid-cols-2 gap-12 items-center">
                    ...
                    <div className="space-y-8">
                      ...
                      {/* Badge */}
                      <div className="inline-flex items-center gap-2 bg-white/10 border border-yellow-500/30 rounded-full px-4 py-2">
                        <Award size={16} className="text-yellow-500" />
                        <span className="text-yellow-500 text-sm">{slide.badge}</span>
                      </div>
                      ...
                      {/* CTA */}
                      <div className="flex flex-wrap gap-4">
                        <button className="bg-yellow-500 hover:bg-yellow-600 text-black px-8 py-4 rounded-lg font-semibold text-lg transition">
                          Book Free Demo
                        </button>
                        <button className="border-2 border-white/30 hover:border-yellow-500 px-8 py-4 rounded-lg font-semibold text-lg transition">
                          Explore Courses
                        </button>
                      </div>
                      ...
                      <div className="flex items-center gap-6 pt-6">
                        ...
                        <div className="flex items-center gap-2">
                          <Star className="text-yellow-500" size={20} fill="currentColor" />
                          <span>Top Rated Training</span>
                        </div>
                      </div>
                    </div>

                    {/* Right Icon */}
                    <div className="hidden md:block text-9xl text-white/10 rotate-12">
                      ♞
                    </div>
        ...
        {/* Controls */}
        <button onClick={prevSlide} className="absolute left-8 top-1/2 -translate-y-1/2 bg-black/30 p-4 rounded-full">
          <ChevronLeft size={24} />
        </button>
        <button onClick={nextSlide} className="absolute right-8 top-1/2 -translate-y-1/2 bg-black/30 p-4 rounded-full">
          <ChevronRight size={24} />
        </button>
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-3">
          {slides.map((_, i) => (
            <button
              ...
              className={`h-2 rounded-full ${i === currentSlide ? 'w-10 bg-yellow-500' : 'w-2 bg-white/50'}`}
            />
          ))}
        </div>
      </div>
      ...
      <div className="container mx-auto px-6 py-16">
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <div className="bg-white/5 rounded-xl p-6 border border-yellow-500/10">
            <Clock className="text-yellow-500 mb-3" />
            <h3 className="font-semibold mb-2">Flexible Timings</h3>
            <p className="text-gray-400 text-sm">Learn at your convenience</p>
          </div>

          <div className="bg-white/5 rounded-xl p-6 border border-yellow-500/10">
            <Shield className="text-yellow-500 mb-3" />
            <h3 className="font-semibold mb-2">Structured Curriculum</h3>
            <p className="text-gray-400 text-sm">Step-by-step learning path</p>
          </div>

          <div className="bg-white/5 rounded-xl p-6 border border-yellow-500/10">
            <Users className="text-yellow-500 mb-3" />
            <h3 className="font-semibold mb-2">Personal Attention</h3>
            <p className="text-gray-400 text-sm">Small batch & 1-on-1 sessions</p>
          </div>
